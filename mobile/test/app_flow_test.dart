...
void main() {
    ...
    final mock = MockClient((request) async {
      ...
      switch (path) {
        case '/api/auth/login':
          ...
          if (payload['password'] != 'Supervisor@123') {
  ...
  Future<void> signIn(
    ...
    String email = 'supervisor@stock.com',
  }) async {
  ...
  testWidgets('signs a supervisor in and renders the dashboard', (tester) async {
    ...
    await signIn(tester, password: 'Supervisor@123');
  ...
  testWidgets('surfaces a rejected login and stays on the login screen',
    ...
    expect(find.text('Invalid credentials. Please try again.'), findsWidgets);
    expect(find.text('Sign In to Portal'), findsOneWidget);
  ...
  testWidgets('turns an admin away at login', (tester) async {
    ...
    await signIn(tester, password: 'Supervisor@123', email: 'admin@stock.com');
