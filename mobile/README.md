...
server. Nothing is shown while the server is reachable.
...
Accounts sign in with their **name** and a **4-digit PIN** — there is no email
or password. The seeder creates the accounts below without a PIN; an Admin
issues each one from **Users & PINs** in the web console before it can be used.

| Role       | Name                    |
| ---------- | ----------------------- |
| Supervisor | `Store Supervisor`      |
| Branch     | `<Room name> Branch`    |
