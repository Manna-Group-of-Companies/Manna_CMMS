...
/**
 * The supervisor's own returns. A return needs no approval — it is in the Red
 * Stock Room straight away — so this screen tracks how far each one has moved
 * through the merge afterwards, and lets the supervisor ask for a merge rather
 * than waiting for the weekly run. The decision stays with the Admin.
 */
const MyReturns = () => {
  ...
  // What a merge request would ask for: Red Stock not already inside one.
  const awaiting = items.filter((item) => item.status === "In Red Stock");
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-5 rounded-2xl glass-premium border border-slate-200">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900">My Returned Stock</h3>
      ...
      {loading ? (
        ...
        <div className="glass-premium p-12 text-center rounded-2xl border border-slate-200 flex flex-col items-center justify-center">
          ...
          <h3 className="text-base font-bold text-slate-900 mb-1">No returns found</h3>
          <p className="text-xs text-slate-500">
            Return issued stock from your Issue History — it goes straight into the Red Stock
            Room.
          </p>
        </div>
      ) : (
